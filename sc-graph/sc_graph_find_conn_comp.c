...
{
    sc_addr not_checked_vertices, conn_comp_set, curr_vertex;
    ...
    while(sc_iterator3_next(it3) == SC_TRUE)
    {
        ...
        if(sc_helper_check_arc(not_checked_vertices, curr_vertex,
            ...
            continue;
        deep_first_search(curr_vertex, not_checked_vertices, curr_conn_comp, graph);
...
void deep_first_search(sc_addr curr_vertex, sc_addr not_checked_vertices, sc_addr curr_conn_comp, sc_addr graph)
{
    ...
    while(sc_iterator5_next(it5) == SC_TRUE)
    {
        if (sc_helper_check_arc(not_checked_vertices, it5->results[2],
            ...
            deep_first_search(it5->results[2], not_checked_vertices, curr_conn_comp, graph);
    }
    sc_iterator3_free(it5);
